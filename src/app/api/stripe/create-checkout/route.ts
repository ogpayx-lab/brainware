...
export async function POST(req: NextRequest) {
  try {
    ...
    // Create checkout session with plan + metered AI
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        ...
        { price: aiPrice }, // metered — no quantity
      ],
      subscription_data: {
        trial_period_days: 30,
        metadata: { user_id: userId, store_id: user.store_id, plan: planId },
      },
      success_url: `${process.env.NEXT_PUBLIC_APP_URL || 'https://brain-ware.ai'}/owner/billing?success=true`,
