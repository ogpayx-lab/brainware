...
// PUT - Acquisisce il pagamento di un ordine (capture)
export async function PUT(req: NextRequest) {
  ...
  try {
    // Step 1: Get existing transactions to find the authorization
    const txRes = await fetch(
      `https://${creds.shopifyDomain}/admin/api/2024-01/orders/${orderId}/transactions.json`,
      { headers: { 'X-Shopify-Access-Token': creds.accessToken }, cache: 'no-store' }
    )
    if (!txRes.ok) return NextResponse.json({ error: `Errore recupero transazioni: ${txRes.status}` }, { status: txRes.status })
    const { transactions } = await txRes.json()

    // Find the authorization transaction
    const authTx = transactions?.find((t: any) => t.kind === 'authorization' && t.status === 'success')

    // Step 2: Create capture transaction
    const captureBody: any = {
      transaction: {
        kind: 'capture',
        ...(amount ? { amount } : {}),
        ...(authTx ? { parent_id: authTx.id } : {}),
      }
    }
    ...
    if (!captureRes.ok) {
      ...
      return NextResponse.json({ error: `Errore acquisizione pagamento: ${captureRes.status}`, detail: err }, { status: captureRes.status })
    }
